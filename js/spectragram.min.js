	...
	var Instagram = {
        ...
        displayPhotos: function ( results ) {
			...
			for ( var i = 0; i < max; i++ ) {
				...
				isImage = ["VIDEO"].includes(results.data[i].media_type);

				if (isImage && [4,5,9].includes(i)) {
					...
					$element = `<div class="col-md-4 ftco-animate">
								    <video class="insta-video-block" src="${results.data[i].media_url}" autoplay loop muted></video>
								</div>`;
			...
			this.$elem.find('video').map((k, video) => {
